...
假如现有一32位`int`型数`0x12345678`，那么其MSB(Most Significant Byte，最高有效字节)为`0x12`，其LSB (Least Significant Byte，最低有效字节)为`0x78`，在CPU内存中有两种存放方式：（假设从地址`0x4000`开始存放），则大小端区别如下：==大小段判断重点：沿着内存地址递增的方向，一个数比如`0x12345678`，在内存中是先存储高位数据还是先存储低位数据，若先存高位数据则为大端，先存低位数据则为小段==
